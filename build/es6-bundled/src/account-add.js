define(["exports","./login-on.js"],function(_exports,_loginOn){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.usageStatistics=_exports.runIfDevelopmentMode=_exports.ThemePropertyMixin=_exports.ThemableMixin=_exports.TextFieldMixin=_exports.TextFieldElement=_exports.SelectElement=_exports.OverlayElement=_exports.Material=_exports.Lumo=_exports.ListMixin=_exports.ListBoxElement=_exports.ItemMixin=_exports.ItemElement=_exports.IronJsonpLibraryBehavior=_exports.FocusablesHelper=_exports.ElementMixin=_exports.ControlStateMixin=_exports.$version$1=_exports.$version=_exports.$vaadinUsageStatistics=_exports.$vaadinThemePropertyMixin=_exports.$vaadinThemableMixin=_exports.$vaadinTextFieldMixin=_exports.$vaadinTextField=_exports.$vaadinSelect=_exports.$vaadinOverlay=_exports.$vaadinListMixin=_exports.$vaadinListBox=_exports.$vaadinItemMixin=_exports.$vaadinItem=_exports.$vaadinFocusablesHelper=_exports.$vaadinElementMixin=_exports.$vaadinDevelopmentModeDetector=_exports.$vaadinControlStateMixin=_exports.$ironJsonpLibrary=void 0;const IronJsonpLibraryBehavior={properties:{/**
...
return}}this.callbackName=callbackName;window[this.callbackName]=this.success.bind(this);this.addScript(url)};/** @constructor */Loader.prototype={callbackMacro:"%%callback%%",loaded:!1,addScript:function(src){var script=document.createElement("script");script.src=src;script.onerror=this.handleError.bind(this);var s=document.querySelector("script")||document.body;s.parentNode.insertBefore(script,s);this.script=script},removeScript:function(){if(this.script.parentNode){this.script.parentNode.removeChild(this.script)}this.script=null},handleError:function(ev){this.error=new Error("Library failed to load");this.notifyAll();this.cleanup()},success:function(){this.loaded=!0;this.result=Array.prototype.slice.call(arguments);this.notifyAll();this.cleanup()},cleanup:function(){delete window[this.callbackName]},notifyAll:function(){this.notifiers.forEach(function(notifyCallback){notifyCallback(this.error,this.result)}.bind(this));this.notifiers=[]},requestNotify:function(notifyCallback){if(this.loaded||this.error){notifyCallback(this.error,this.result)}else{this.notifiers.push(notifyCallback)}}};/**
   ...
   */(0,_loginOn.Polymer)({is:"iron-jsonp-library",behaviors:[IronJsonpLibraryBehavior],properties:{/**
     * Library url. Must contain string `%%callback%%`.
     *
     * `%%callback%%` is a placeholder for jsonp wrapper function name
     *
     ...
     */notifyEvent:String/**
                            ...
                            * will fire upon successful load
                            * @event `notifyEvent`
                            */}});var ironJsonpLibrary={IronJsonpLibraryBehavior:IronJsonpLibraryBehavior};_exports.$ironJsonpLibrary=ironJsonpLibrary;(0,_loginOn.Polymer)({is:"google-maps-api",behaviors:[IronJsonpLibraryBehavior],properties:{/** @private */mapsUrl:{type:String,value:"https://maps.googleapis.com/maps/api/js?callback=%%callback%%"},/**
...
if(!apiKey&&!clientId){const warning="No Google Maps API Key or Client ID specified. "+"See https://developers.google.com/maps/documentation/javascript/get-api-key "+"for instructions to get started with a key or client id.";console.warn(warning)}if(language){url+=`&language=${language}`}if(signedIn){url+=`&signed_in=${signedIn}`}return url},/**
   ...
   */get api(){return google.maps}});function setupDragHandler_(){if(this.draggable){this.dragHandler_=google.maps.event.addListener(this.marker,"dragend",onDragEnd_.bind(this))}else{google.maps.event.removeListener(this.dragHandler_);this.dragHandler_=null}}function onDragEnd_(e,details,sender){this.latitude=e.latLng.lat();this.longitude=e.latLng.lng()}(0,_loginOn.Polymer)({_template:_loginOn.html$1`
    <style>
      :host {
        display: none;
      }
...
break;case"postal_code":objetoGeo.postcode=v.short_name;break;}});objetoGeo.oo=results[0];this.set("geocoder",objetoGeo)}})}});(0,_loginOn.Polymer)({_template:_loginOn.html$1`
    <style>
      :host {
        position: relative;
        display: block;
...
const markerIndex=this.$.selector.indexOf(e.target);if(this.singleInfoWindow){this.$.selector.selected=null}else if(this.$.selector.selectedValues){this.$.selector.selectedValues=this.$.selector.selectedValues.filter(i=>i!==markerIndex)}}});(0,_loginOn.Polymer)({_template:_loginOn.html$1`
    <style>
      :host {
        display: block;
      }
  ...
  `,is:"gold-phone-input",behaviors:[_loginOn.PaperInputBehavior,_loginOn.IronFormElementBehavior],properties:{/**
...
if(this.is){window.Vaadin.registrations.push(this);if(window.Vaadin.developmentModeCallback){statsJob=_loginOn.Debouncer.debounce(statsJob,_loginOn.idlePeriod,()=>{window.Vaadin.developmentModeCallback["vaadin-usage-statistics"]()});(0,_loginOn.enqueueDebouncer)(statsJob)}}}ready(){super.ready();if(null===document.doctype){console.warn("Vaadin components require the \"standards mode\" declaration. Please add <!DOCTYPE html> to the HTML document.")}}};_exports.ElementMixin=ElementMixin;var vaadinElementMixin={ElementMixin:ElementMixin};/**
   @license
   Copyright (c) 2017 Vaadin Ltd.
   This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
   */ /**
       ...
       * @polymerMixin
       */_exports.$vaadinElementMixin=vaadinElementMixin;const ItemMixin=superClass=>class VaadinItemMixin extends superClass{static get properties(){return{/**
...
this._includeStyle(defaultModuleName,template)}}/** @private */static _includeStyle(moduleName,template){if(template&&!template.content.querySelector(`style[include="${moduleName}"]`)){const styleEl=document.createElement("style");styleEl.setAttribute("include",moduleName);template.content.appendChild(styleEl)}}};_exports.ThemableMixin=ThemableMixin;var vaadinThemableMixin={ThemableMixin:ThemableMixin};_exports.$vaadinThemableMixin=vaadinThemableMixin;class ItemElement extends ItemMixin(ThemableMixin(_loginOn.PolymerElement)){static get template(){return _loginOn.html$1`
    <style>
      :host {
...
`}static get is(){return"vaadin-item"}static get version(){return"2.1.0"}}_exports.ItemElement=ItemElement;customElements.define(ItemElement.is,ItemElement);var vaadinItem={ItemElement:ItemElement};_exports.$vaadinItem=vaadinItem;class Lumo extends HTMLElement{static get version(){return"1.4.2"}}_exports.Lumo=Lumo;customElements.define("vaadin-lumo-styles",Lumo);var version={Lumo:Lumo};_exports.$version=version;const $_documentContainer=document.createElement("template");$_documentContainer.innerHTML=`<custom-style>
  <style>
    @font-face {
...
</dom-module>`;document.head.appendChild($_documentContainer$5.content);class Material extends HTMLElement{static get version(){return"1.2.2"}}_exports.Material=Material;customElements.define("vaadin-material-styles",Material);var version$1={Material:Material};_exports.$version$1=version$1;const $_documentContainer$6=document.createElement("template");$_documentContainer$6.innerHTML=`<custom-style>
  <style>
    @font-face {
...
</custom-style>`;document.head.appendChild($_documentContainer$7.content);const font="https://fonts.googleapis.com/css?family=Roboto+Mono:400,700|Roboto:400,300,300italic,400italic,500,500italic,700,700italic",link=document.createElement("link");link.rel="stylesheet";link.type="text/css";link.crossOrigin="anonymous";link.href=font;document.head.appendChild(link);const $_documentContainer$8=_loginOn.html$1`<custom-style>
  <style>
    html {
      /* Font family */
...
_scrollToItem(idx){const item=this.items[idx];if(!item){return}const props=this._vertical?["top","bottom"]:["left","right"],scrollerRect=this._scrollerElement.getBoundingClientRect(),nextItemRect=(this.items[idx+1]||item).getBoundingClientRect(),prevItemRect=(this.items[idx-1]||item).getBoundingClientRect();let scrollDistance=0;if(nextItemRect[props[1]]>=scrollerRect[props[1]]){scrollDistance=nextItemRect[props[1]]-scrollerRect[props[1]]}else if(prevItemRect[props[0]]<=scrollerRect[props[0]]){scrollDistance=prevItemRect[props[0]]-scrollerRect[props[0]]}this._scroll(scrollDistance)}/* @protected */get _vertical(){return"horizontal"!==this.orientation}_scroll(pixels){this._scrollerElement["scroll"+(this._vertical?"Top":"Left")]+=pixels}};_exports.ListMixin=ListMixin;var vaadinListMixin={ListMixin:ListMixin};_exports.$vaadinListMixin=vaadinListMixin;class ListBoxElement extends ElementMixin(ListMixin(ThemableMixin(_loginOn.PolymerElement))){static get template(){return _loginOn.html$1`
    <style>
      :host {
        display: flex;
      }
...
/** @private */const klass=(()=>class extends HTMLElement{static get is(){return is}connectedCallback(){if(window.ShadyCSS){window.ShadyCSS.styleElement(this)}if(!this.shadowRoot){this.attachShadow({mode:"open"});this.shadowRoot.appendChild(document.importNode(styledTemplate.content,!0))}}})();customElements.define(klass.is,klass);overlayContentCache[cssText]=is;return is};/**
    ...
    * @demo demo/index.html
    */class OverlayElement extends ThemableMixin(_loginOn.PolymerElement){static get template(){return _loginOn.html$1`
    <style>
      :host {
        z-index: 200;
        position: fixed;
...
`}static get is(){return"vaadin-overlay"}static get properties(){return{opened:{type:Boolean,notify:!0,observer:"_openedChanged",reflectToAttribute:!0},/**
       ...
       */restoreFocusOnClose:{type:Boolean,value:!1},_mouseDownInside:{type:Boolean},_mouseUpInside:{type:Boolean},_instance:{type:Object},_originalContentPart:Object,_contentNodes:Array,_oldOwner:Element,_oldModel:Object,_oldTemplate:Object,_oldInstanceProps:Object,_oldRenderer:Object,_oldOpened:Boolean}}static get observers(){return["_templateOrRendererChanged(template, renderer, owner, model, instanceProps, opened)"]}constructor(){super();this._boundMouseDownListener=this._mouseDownListener.bind(this);this._boundMouseUpListener=this._mouseUpListener.bind(this);this._boundOutsideClickListener=this._outsideClickListener.bind(this);this._boundKeydownListener=this._keydownListener.bind(this);this._observer=new _loginOn.FlattenedNodesObserver(this,info=>{this._setTemplateFromNodes(info.addedNodes)});// Listener for preventing closing of the paper-dialog and all components extending `iron-overlay-behavior`.
...
this.$.content.parentNode.replaceChild(this._originalContentPart,this.$.content);this.$.content=this._originalContentPart;this._originalContentPart=void 0}this._observer.connect();this._contentNodes=void 0;this.content=void 0}_stampOverlayTemplate(template,instanceProps){this._removeOldContent();if(!template._Templatizer){template._Templatizer=(0,_loginOn.templatize)(template,this,{instanceProps:instanceProps,forwardHostProp:function(prop,value){if(this._instance){this._instance.forwardHostProp(prop,value)}}})}this._instance=new template._Templatizer({});this._contentNodes=Array.from(this._instance.root.childNodes);const templateRoot=template._templateRoot||(template._templateRoot=template.getRootNode()),_isScoped=templateRoot!==document;if(_isScoped){if(!this.$.content.shadowRoot){this.$.content.attachShadow({mode:"open"})}let scopeCssText="";const host=templateRoot.host;if(host&&host._template){scopeCssText=(0,_loginOn.stylesFromTemplate)(host._template).reduce((result,style)=>result+style.textContent,"")}// The overlay root’s :host styles should not apply inside the overlay
...
while(n&&n!==doc&&n!==this){n=n.parentNode||n.host}return n===this}}_exports.OverlayElement=OverlayElement;customElements.define(OverlayElement.is,OverlayElement);var vaadinOverlay={OverlayElement:OverlayElement};_exports.$vaadinOverlay=vaadinOverlay;const $_documentContainer$m=document.createElement("template");$_documentContainer$m.innerHTML=`<dom-module id="vaadin-select-overlay-styles" theme-for="vaadin-select-overlay">
  <template>
    <style>
      :host {
...
this.setAttribute("input-prevented","");this._inputDebouncer=_loginOn.Debouncer.debounce(this._inputDebouncer,_loginOn.timeOut.after(200),()=>{this.removeAttribute("input-prevented")});return}}this.__userInput=!0;this.value=e.target.value}// NOTE(yuriy): Workaround needed for IE11 and Edge for proper displaying
...
const isSafari=/^((?!chrome|android).)*safari/i.test(navigator.userAgent);/* istanbul ignore if */if(isSafari&&this.root){const WEBKIT_PROPERTY="-webkit-backface-visibility";this.root.querySelectorAll("*").forEach(el=>{el.style[WEBKIT_PROPERTY]="visible";el.style[WEBKIT_PROPERTY]=""})}}/**
     ...
     *
     * @event change
     */};_exports.TextFieldMixin=TextFieldMixin;var vaadinTextFieldMixin={TextFieldMixin:TextFieldMixin};_exports.$vaadinTextFieldMixin=vaadinTextFieldMixin;class TextFieldElement extends ElementMixin(TextFieldMixin(ThemableMixin(_loginOn.PolymerElement))){static get template(){return _loginOn.html$1`
...
`}static get is(){return"vaadin-text-field"}static get version(){return"2.3.7"}static get properties(){return{/**
       ...
       */title:{type:String}}}}_exports.TextFieldElement=TextFieldElement;customElements.define(TextFieldElement.is,TextFieldElement);var vaadinTextField={TextFieldElement:TextFieldElement};_exports.$vaadinTextField=vaadinTextField;let memoizedTemplate;/**
  ...
  <style>
    @font-face {
      ...
      font-style: normal;
                                                          ...
                                                          * @demo demo/index.html
                                                          */class SelectElement extends ElementMixin(ControlStateMixin(ThemableMixin(ThemePropertyMixin((0,_loginOn.mixinBehaviors)(_loginOn.IronResizableBehavior,_loginOn.PolymerElement))))){static get template(){return _loginOn.html$1`
    <style>
      :host {
        display: inline-block;
      }
...
super._setFocused(this.opened||focused);this.focusElement._setFocused(this.hasAttribute("focused"));this.hasAttribute("focused")||this.validate()}_setPosition(){const inputRect=this._inputElement.shadowRoot.querySelector("[part~=\"input-field\"]").getBoundingClientRect(),viewportHeight=Math.min(window.innerHeight,document.documentElement.clientHeight),bottomAlign=inputRect.top>(viewportHeight-inputRect.height)/2;this._overlayElement.style.left=inputRect.left+"px";if(bottomAlign){this._overlayElement.setAttribute("bottom-aligned","");this._overlayElement.style.removeProperty("top");this._overlayElement.style.bottom=viewportHeight-inputRect.bottom+"px"}else{this._overlayElement.removeAttribute("bottom-aligned");this._overlayElement.style.removeProperty("bottom");this._overlayElement.style.top=inputRect.top+"px"}this._overlayElement.updateStyles({"--vaadin-select-text-field-width":inputRect.width+"px"})}/**
     ...
     */validate(){return!(this.invalid=!(this.disabled||!this.required||this.value))}}_exports.SelectElement=SelectElement;customElements.define(SelectElement.is,SelectElement);var vaadinSelect={SelectElement:SelectElement};_exports.$vaadinSelect=vaadinSelect;const $_documentContainer$p=_loginOn.html$1`<dom-module id="lumo-text-field" theme-for="vaadin-text-field">
  <template>
...
</dom-module>`;document.head.appendChild($_documentContainer$p.content);const $_documentContainer$q=_loginOn.html$1`<dom-module id="lumo-select" theme-for="vaadin-select">
  <template>
...
</dom-module>`;document.head.appendChild($_documentContainer$q.content);const $_documentContainer$r=_loginOn.html$1`<dom-module id="material-text-field" theme-for="vaadin-text-field">
  <template>
...
</dom-module>`;document.head.appendChild($_documentContainer$r.content);const $_documentContainer$s=_loginOn.html$1`<dom-module id="material-select" theme-for="vaadin-select">
  <template>
...
</dom-module>`;document.head.appendChild($_documentContainer$s.content);class AccountAdd extends _loginOn.PolymerElement{static get template(){return _loginOn.html`

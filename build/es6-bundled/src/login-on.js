...
// in `index.html`.
setRootPath(MainInitGlobals.rootPath);class LoginOn extends PolymerElement{static get template(){return html`
        <style>
            :host {
    ...
    `}ready(){super.ready();this.$.frmLoginOn.addEventListener("iron-form-presubmit",e=>{e.preventDefault();firebase.auth().signInWithEmailAndPassword(this.login.username,this.login.password).then(result=>{this.updateStyles({"--background-color":"#eeeeee"});this.set("login.uid",result.user.uid);this.set("page","main");new Promise((res,rej)=>_require.default(["./main-init.js"],res,rej)).then(bundle=>bundle&&bundle.$mainInit||{})}).catch(error=>{this.$.tostadaError.text="["+error.code+"] "+error.message;this.$.tostadaError.open()})})}static get properties(){return{page:{type:String,reflectToAttribute:!0,value:"login"},login:{type:Object,value:{username:"",password:"",uid:""}}}}static get observers(){return[// '__checkLogin(login.uid)'
]}__loginOn(){this.$.frmLoginOn.submit()}}window.customElements.define("login-on",LoginOn);const $_documentContainer$1=document.createElement("template");$_documentContainer$1.innerHTML=`<dom-module id="shared-styles">
  <template>
    <style>
      .card {
        color: #757575;

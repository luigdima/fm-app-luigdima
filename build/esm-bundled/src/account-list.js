import{IronFormElementBehavior,IronValidatableBehavior,PaperInkyFocusBehavior,PaperInkyFocusBehaviorImpl,PaperRippleBehavior,Polymer,html$1 as html,afterNextRender,IronOverlayBehavior,dom,PolymerElement,html as html$1}from"./login-on.js";const IronCheckedElementBehaviorImpl={properties:{/**
...
let callbackMethod="on"+eventLowerCase;if("function"===typeof this[callbackMethod]){this[callbackMethod](browserEvent)}}setDefaultPopupParent(){if(!this.defaultPopupParentSet&&this.api&&!this._gridOptions.popupParent){this.defaultPopupParentSet=!0;this.api.setPopupParent(this.querySelector(".ag-root"))}}hypenateAndLowercase(property){return property.replace(/([a-z])([A-Z])/g,"$1-$2").toLowerCase()}}customElements.define("ag-grid-polymer",AgGridPolymer);var agGridPolymer={default:AgGridPolymer};class AccountList extends PolymerElement{static get template(){return html$1`
...
setTimeout(()=>{this.__eliminarEmpresa()},10)}}__retornoEliminar(e){this.textoTostada="["+e.detail.response.Message+"]";this.codeTostada="ID_ENTITY: "+e.detail.response.id+" ";this.$.tostadaSuccess.open();this.$.ventanaEmpresa.close();this.__callListAccounts()}__onErrorEliminar(e){if("error"==e.type){var respuesta=e.detail.request.response;this.$.tostadaError.open();this.textoTostada="["+e.detail.request.response.error+"]";this.codeTostada="Code: "+e.detail.request.response.code+" "}}}window.customElements.define("account-list",AccountList);export{BaseFrameworkComponent as $BaseFrameworkComponent,BaseGuiComponent$1 as $BaseFrameworkComponentDefault,PolymerComponentFactory$1 as $PolymerComponentFactory,PolymerComponentFactory as $PolymerComponentFactoryDefault,PolymerFrameworkComponentWrapper$1 as $PolymerFrameworkComponentWrapper,PolymerFrameworkComponentWrapper as $PolymerFrameworkComponentWrapperDefault,PolymerFrameworkFactory$1 as $PolymerFrameworkFactory,PolymerFrameworkFactory as $PolymerFrameworkFactoryDefault,agGridPolymer as $agGridPolymer,AgGridPolymer as $agGridPolymerDefault,ironCheckedElementBehavior as $ironCheckedElementBehavior,neonAnimatableBehavior as $neonAnimatableBehavior,neonAnimationRunnerBehavior as $neonAnimationRunnerBehavior,paperCheckedElementBehavior as $paperCheckedElementBehavior,paperDialogBehavior as $paperDialogBehavior,IronCheckedElementBehavior,IronCheckedElementBehaviorImpl,NeonAnimatableBehavior,NeonAnimationRunnerBehavior,NeonAnimationRunnerBehaviorImpl,PaperCheckedElementBehavior,PaperCheckedElementBehaviorImpl,PaperDialogBehavior,PaperDialogBehaviorImpl};